...
class Command(BaseCommand):
    help = 'Insert 18 practice slots into the database'
    ...
    def handle(self, *args, **options):
        ...
        for slot_text in practice_slots:
            ...
            if not PracticeSlot.objects.filter(practice_slot=slot_text).exists():
                PracticeSlot.objects.create(practice_slot=slot_text)
                created_count += 1
                self.stdout.write(self.style.SUCCESS(f'Created: {slot_text}'))
            else:
                self.stdout.write(self.style.WARNING(f'Already exists: {slot_text}'))

        self.stdout.write(self.style.SUCCESS(f'\nTotal: {created_count} new practice slots created'))
